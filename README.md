...
Modern terminal UI toolkit: ANSI styling, Unicode tables, callout boxes, progress bars, and prompts
...
| `new_config(name, count)` | `name = "World", count = 1` | `TermConfig` | Constructs a new configuration struct. |
| `core_format_greeting(name)` | `name` | `string` | Core formatter producing `Hello, {name}!`. |
| `core_format_custom(config)` | `config: TermConfig` | `string` | Formats greeting using prefix and name from config. |
